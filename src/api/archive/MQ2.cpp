...
#include "MQ2.h"
  ...
  float MQ2::readLPG(){
      if (millis()<(lastReadTime + 10000) && lpg != 0){
          ...
          return lpg = MQGetGasPercentage(MQRead()/10,GAS_LPG);  //  TODO:  /10 should be /Ro je
      }
  }
  ...
  int MQ2::MQGetPercentage(float rs_ro_ratio, float *pcurve) {
    return (pow(10,(((log10(rs_ro_ratio)-pcurve[1])/pcurve[2]) + pcurve[0])));    //   TODO:  modified log to log10.   should verify if this change is needed elsewhere
  }
